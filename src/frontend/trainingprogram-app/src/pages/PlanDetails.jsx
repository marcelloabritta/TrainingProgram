...
import MonthCard from "../components/plan/MonthCard";
...
function PlanDetails() {
  ...
  useEffect(() => {
    if(plan) {
      setTitle(`${plan.Year} Season`);
      setShowBackButton(true);
    }

    if (plan && plan.Microcycles && plan.Microcycles.length > 0) {
      const sortedWeeks = [...plan.Microcycles].sort(
      ...
      const monthArray = Object.keys(groups).map((index) => {
        ...
        const monthName = format(
          addMonths(planStartDate, monthIndex),
          "MMMM yyyy",
          { locale: enUS }
        );
  ...
  }, [plan, setTitle, setShowBackButton]);

  if (loading) return <p className="text-white p-6 text-center">Loading your plan...</p>;
